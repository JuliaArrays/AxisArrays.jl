...
@test @inferred(collapse(Val{2}, (:A1, :A2), A1, A2)) == AxisArray(cat(3, A1data, A2data), Axis{:X}(1:2), Axis{:Y}(1:2), Axis{:collapsed}(AxisArrays.CategoricalVector([(:A1,), (:A2, :foo)])))
...
@test @inferred(collapse(Val{0}, A1)) == AxisArray(vec(A1data), Axis{:collapsed}(AxisArrays.CategoricalVector(collect(IterTools.product((1,), axisvalues(A1)...)))))
@test @inferred(collapse(Val{1}, A1)) == AxisArray(A1data, Axis{:row}(1:2), Axis{:collapsed}(AxisArrays.CategoricalVector(collect(IterTools.product((1,), axisvalues(A1)[2])))))
